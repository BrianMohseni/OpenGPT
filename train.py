...
def main():
    ...
    parser.add_argument("--enable_fsdp", action="store_true", default=True,
                        help="boolean: enable/disable sharded model")
    ...
    parser.add_argument("--dataset_type", type=str, default="pretraining",
                        help="pretrain for pretraining, posttrain for conversational json")
    parser.add_argument("--savepath", type=str, default="model.pt",
                        help="savepath for weights from training, should end in pt.")
    ...
    # state_dict = load_file("ftm.pt", weights_only=True)
    #sd = load_file("model_checkpoint/model.safetensors")
    ...
    # optimizer = torch.optim.LBFGS(lr=.01, history_size=5)
    model_instance, optimizer, train_dataloader = accelerator.prepare(model_instance, optimizer, train_dataloader)

    model_instance.train()
    step = 0
    # progbar = tqdm(train_dataloader, total=args.train_steps, desc="Training")
    for batch in train_dataloader:  # enumerate(progbar):
        inputs = batch[:, :-1]
        targets = batch[:, 1:]
        ...
        # progbar.set_postfix(step=step, loss=f"loss.item():.5f")
